...
const (
	...
	nodeMeasureQueryTemplate = "node_network_receive_bytes_total{instance=\"%s\",device=\"%s\"}"
)
...
func (p *PrometheusHandle) GetNodeBandwidthMeasure(node string) (*model.Sample, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[NetworkTraffic] Error querying prometheus: %w, node: %v", err, node)
	}

	nodeMeasure := res.(model.Vector)
	if len(nodeMeasure) != 1 {
		return nil, fmt.Errorf("[NetworkTraffic] Invalid response, expected 1 value, got %d, node: %v", len(nodeMeasure), node)
	}
...
func getNodeBandwidthQuery(node, networkInterface string, timeRange time.Duration) string {
	return fmt.Sprintf(nodeMeasureQueryTemplate, node, networkInterface)
}
